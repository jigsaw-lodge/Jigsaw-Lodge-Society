...
| Security | Enforce backend startup failure without `ADMIN_TOKEN`, and keep unique tokens per environment. | Security | Needs Re-Verify | Startup failure is now proven by `test/runtime-config.test.js`, and `docs/security-review.md` now records the current token truth, but unique token hygiene across every environment still needs operational proof. |
| Security | Enforce rate limiting, CORS, and player request authentication for the current build. | Security | Verified (2026-04-09) | Current suite passed `33/33`, and `docs/security-review.md` now records the present auth/CORS scope: signed requests are implemented, replay and stale rejection work, rate limiting is active, and open CORS is still a deliberate non-final posture. |
